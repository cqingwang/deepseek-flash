...
已删除：`scripts/` 目录全部脚本（dspark-vllm.sh、install-autostart.sh、repro-preflight.sh、
longctx-verify.py、.env.dspark.example —— 功能均已并入 program.py / dspark.env.json）。
...
用法一：管理与部署 / 自检（head 上执行）
   ./deploy.sh --install [模型] / --uninstall / --restart / --live_check / --chat_verify / --preflight
...
**职责划分**：
- `program.py` 按本机 hostname 识别 head/worker 角色：head 上 `start`=worker 先起(head 经
  SSH 触发 worker `ensure`)→ head 后起；head 上 `stop`=head 先停 → worker 后停（幂等）；
  worker 上 `ensure`=拉起 worker 容器（systemd 开机守护，幂等）。
...
| 自检（head） | `preflight [worker目标]` | 双机环境自检：SSH/GPU/CUDA/镜像/模型/RoCE/端口，FAIL 计数决定退出码 |
...
```python
def announce(action, desc="", level="INFO"):
    """统一日志：时间戳 + level + action[: desc]；ERROR 打印后退出应用。"""
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"[{ts}] [{level}] {action}" + (f" — {desc}" if desc else ""), flush=True)
    if level == "ERROR":
        sys.exit(1)
```

- level：`INFO / WARN / ERROR`；ERROR 打印后进程以非 0 退出（`fail()` 为其薄包装）。
- **过程日志**（动作/进度/警告/错误）统一走 announce；**数据输出**（gen-env 的 .env 内容、
  load-config 导出行、chat_verify JSON 报告、preflight `[OK]/[FAIL]` 清单、help 文本）
...
`load_config`（pyyaml 缺失提示）/ `cfg_constants`（派生常量）/ `ssh_run` / `scp_to_worker` /
...
```bash
./deploy.sh --preflight                 # 部署前自检（FAIL=0 才可部署）
...
- 命令分发：`--help` / `--preflight` / 无前缀命令 / 未知命令报错
